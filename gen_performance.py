...
BASE_FOLDER = Path('/home/diegopc/projects/robot_formation_in_turbulence/results/performance')
BOUNDS = [-5, 5]
DT = 0.066


def plot_error(data, title, labels, name, results_folder, y_label='', y_lim=None, legend_dict=None, colors=None, smooth=False, linestyle=None):
    fig = plt.figure()
    ...
    for i in range(num_plots):
        ...
        ax.plot(x, mean_data[i, :], label=labels[i], color=colors[i], linestyle=linestyle[i])
...
if __name__ == '__main__':
    ...
    for metric in ['position_error', 'velocity_error', 'velocity_dir_error', 'pred_actions']:
        data = data_dict[metric]
        num_bs, num_eval, num_robots, total_t = data.shape[:4]
        ...
        if metric == 'position_error':
            data = np.transpose(data, (1, 2, 0, 3)).reshape(-1, num_bs, total_t)
            plot_error(
                ...
                colors=['red', 'purple'], #linestyle=['solid', 'dashed'],
            )
        ...
        elif metric == 'pred_actions':

            #import pdb
            #pdb.set_trace()
            data = np.transpose(np.linalg.norm(data, axis=-1), (1, 2, 0, 3)).reshape(-1, num_bs, total_t)
            plot_error(
                data, title=metric.capitalize().replace('_', ' '), name=metric, labels=list(ablation_folders.keys()),
                #y_lim=[-0.05, 1.05], y_label='Error (m)',
                results_folder=BASE_FOLDER,
